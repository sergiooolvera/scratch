...
import { LogOut, GraduationCap, LayoutDashboard, UserPlus, Users, BookOpen, BadgeCheck, MessageSquare } from 'lucide-react'
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between h-16">
                    <div className="flex">
                        ...
                        {user && (
                            <div className="ml-4 sm:ml-8 flex items-center space-x-1 sm:space-x-2 overflow-x-auto">
                                {profile?.rol !== 'admin' && (
                                ...
                                {profile?.rol === 'profesor' && (
                                    <>
                                        <Link href="/profesor/subir-curso" className={navItemClass('/profesor/subir-curso')}>
                                            <BookOpen className="h-4 w-4" /> <span>Subir Curso</span>
                                        </Link>
                                        <Link href="/profesor/preguntas" className={navItemClass('/profesor/preguntas')}>
                                            <MessageSquare className="h-4 w-4" /> <span>Dudas de Alumnos</span>
                                        </Link>
                                        <Link href="/profesor/ventas" className={navItemClass('/profesor/ventas')}>
                                            <BookOpen className="h-4 w-4" /> <span>Mis Ventas</span>
                                        </Link>
                                    </>
                                )}
                                {profile?.rol === 'admin' && (
                        ...
                        {user ? (
                            <div className="flex items-center space-x-4">
                                <div className="hidden md:flex flex-col items-end">
                                    <span className="text-sm font-medium text-gray-900">{profile?.nombre || user.email}</span>
                                    <span className="text-xs text-gray-500 capitalize">{profile?.rol || 'Alumno'}</span>
                                </div>
                                <button
                                    onClick={handleLogout}
                                    className="flex items-center space-x-2 text-red-600 hover:text-white border border-red-600 hover:bg-red-600 px-4 py-2 rounded-full text-sm font-medium transition-colors"
                                >
