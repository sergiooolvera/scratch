...
export default function Navbar() {
    ...
    const getNavbarBgColor = () => {
        if (!user && !profile) return 'bg-white';
        const rol = profile?.rol || 'alumno';
        if (['alumno', 'financiero'].includes(rol)) return 'bg-blue-100';
        if (['instructor', 'capacitador'].includes(rol)) return 'bg-green-100';
        if (['institucion'].includes(rol)) return 'bg-orange-100';
        return 'bg-white';
    };

    const navBgClass = getNavbarBgColor();

    return (
        <nav className={`${navBgClass} border-b border-gray-100 sticky top-0 z-50 print:hidden transition-colors duration-300`}>
                            ...
                            <div className="hidden md:flex ml-8 items-center space-x-2">
                                ...
                                {(profile?.rol === 'instructor' || profile?.rol === 'capacitador' || profile?.rol === 'institucion') && (
                                    <>
                                        <div className="relative group">
                                            ...
                                            <div className={`absolute left-0 mt-0 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 transition-all duration-200 z-[100] pt-2 ${isProfMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible group-hover:opacity-100 group-hover:visible'}`}>
                                                <div className="py-1 bg-white rounded-md border border-gray-100" role="menu">
                                                    ...
                                                    <Link href="/profesor/revision-cuestionarios" onClick={() => setIsProfMenuOpen(false)} className="block px-4 py-2 text-sm text-gray-700 hover:bg-blue-50 hover:text-blue-700">Revisión de Cuestionarios</Link>
                                        ...
                                        <Link href="/profesor/subir-curso" className="flex items-center space-x-1.5 px-4 py-2 rounded-xl text-sm font-bold bg-orange-600 hover:bg-orange-700 text-white shadow-md hover:shadow-lg transition-all duration-200">
            ...
            {/* Mobile Menu Content */}
            {isMenuOpen && (
                <div className={`md:hidden border-t border-gray-100 ${navBgClass}`}>
