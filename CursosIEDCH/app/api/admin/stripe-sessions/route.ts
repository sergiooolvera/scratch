...
export async function GET() {
    try {
        ...
        if (profile?.rol !== 'admin' && profile?.rol !== 'financiero' && profile?.rol !== 'instructor' && profile?.rol !== 'vendedor' && profile?.rol !== 'capacitador' && profile?.rol !== 'institucion') return NextResponse.json({ error: 'Permisos insuficientes' }, { status: 403 });
        ...
        if (profile?.rol === 'instructor' || profile?.rol === 'capacitador' || profile?.rol === 'institucion') {
