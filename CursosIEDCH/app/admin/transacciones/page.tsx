...
export default function AdminTransaccionesStripePage() {
    ...
    const filtered = transactions.filter(t => {
        ...
        const matchesSearch = !searchVal || 
            (t.customer_email && t.customer_email.toLowerCase().includes(searchVal)) ||
            (t.customer_name && t.customer_name.toLowerCase().includes(searchVal)) ||
            (t.curso_titulo && t.curso_titulo.toLowerCase().includes(searchVal)) ||
            (t.method && t.method.toLowerCase().includes(searchVal))
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
                ...
                <div className="flex gap-2">
                    <button 
                        onClick={async () => {
                            if (!window.confirm("Esto sincronizará los pagos exitosos de Stripe que no estén en la base de datos (por ejemplo pagos OXXO recientes). ¿Deseas continuar?")) return;
                            setLoading(true);
                            try {
                                const res = await fetch('/api/cron/sync-stripe');
                                const data = await res.json();
                                alert(data.message + (data.missingPurchasesRecovered !== undefined ? `\nPagos recuperados: ${data.missingPurchasesRecovered}` : ''));
                                fetchTransactions();
                            } catch (e) {
