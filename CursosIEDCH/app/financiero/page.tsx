    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="mb-8">
                ...
                <p className="mt-2 text-sm text-gray-500">Reportes de ingresos día a día y mes a mes.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col relative overflow-hidden group">
                    <div className="absolute -right-6 -top-6 bg-blue-50 text-blue-100 rounded-full w-24 h-24 flex items-center justify-center opacity-50 group-hover:scale-110 transition-transform">
                        <DollarSign className="w-12 h-12 text-blue-200" />
                    </div>
                    <p className="text-sm font-medium text-gray-500 relative z-10">Ingresos Hoy</p>
                    <p className="text-3xl font-bold text-gray-900 mt-2 relative z-10">${stats.today.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN</p>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col relative overflow-hidden group">
                    <div className="absolute -right-6 -top-6 bg-emerald-50 text-emerald-100 rounded-full w-24 h-24 flex items-center justify-center opacity-50 group-hover:scale-110 transition-transform">
                        <TrendingUp className="w-12 h-12 text-emerald-200" />
                    </div>
                    <p className="text-sm font-medium text-gray-500 relative z-10">Ingresos Este Mes</p>
                    <p className="text-3xl font-bold text-gray-900 mt-2 relative z-10">${stats.thisMonth.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN</p>
                </div>
                <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col relative overflow-hidden group">
                    <div className="absolute -right-6 -top-6 bg-purple-50 text-purple-100 rounded-full w-24 h-24 flex items-center justify-center opacity-50 group-hover:scale-110 transition-transform">
                        <Landmark className="w-12 h-12 text-purple-200" />
                    </div>
                    <p className="text-sm font-medium text-gray-500 relative z-10">Ingresos Totales (Histórico)</p>
                    <p className="text-3xl font-bold text-gray-900 mt-2 relative z-10">${stats.total.toLocaleString('es-MX', { minimumFractionDigits: 2 })} MXN</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {monthlyData.slice().reverse().map((d) => (
                                    <tr key={d.month} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-700">{d.month}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-right">${d.amount.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
                ...
                {/* Ingresos por Día (Últimos días) */}
                <div className="bg-white shadow-sm border border-gray-100 rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-6">
                        <BarChart3 className="w-5 h-5 text-blue-500" />
                        <h2 className="text-lg font-bold text-gray-900">Ingresos Diarios (Últimos días activos)</h2>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {dailyData.map((d) => (
                                    <tr key={d.date} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-700">{d.date}</td>
                                        <td className="px-4 py-3 whitespace-nowrap text-sm font-bold text-gray-900 text-right">${d.amount.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</td>
                                    </tr>
                                ))}
                            </tbody>
