...
test.describe('Módulo de Profesor y Creación/Edición de Cursos', () => {
  ...
  test('Debe permitir agregar módulos y temas en el Temario', async ({ page }) => {
    ...
    // Descartar borrador previo si existe para empezar limpio
    const btnDescartarBorrador = page.locator('button:has-text("Descartar"), button:has-text("Empezar de cero")');
    if (await btnDescartarBorrador.isVisible()) {
      await btnDescartarBorrador.click();
    }
    ...
    // Botón Agregar módulo dentro de la sección de Temario
    const btnAgregarModulo = page.locator('button:has-text("Agregar módulo"):visible').first();
    ...
    const inputTituloModulo = page.locator('input[placeholder="Título del módulo"]:visible').first();
    ...
    const inputTema1 = page.locator('input[placeholder="Tema 1"]:visible').first();
    await expect(inputTema1).toBeVisible();
    await inputTema1.fill('Historia clínica y antecedentes');

    const btnAgregarTema = page.locator('button:has-text("Agregar tema"):visible').first();
    await expect(btnAgregarTema).toBeVisible();
    await btnAgregarTema.click();

    // Verificar que se haya añadido un nuevo input de tema
    const inputsTemas = page.locator('input[placeholder^="Tema "]:visible');
  ...
  test('Debe mostrar la sección "Competencias a desarrollar" con chips de verbos taxonómicos y contador de caracteres', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Identificar"):visible').first()).toBeVisible();
    await expect(page.locator('button:has-text("Reconocer"):visible').first()).toBeVisible();
    await expect(page.locator('button:has-text("Describir"):visible').first()).toBeVisible();
    await expect(page.locator('button:has-text("Comprender"):visible').first()).toBeVisible();

    // Verificar presencia de inputs de competencias iniciales y contador
    const compInputs = page.locator('input[placeholder*="Selecciona un verbo o escribe"]:visible');
  ...
  test('Debe permitir insertar verbos con un clic, agregar hasta 5 competencias y eliminar filas', async ({ page }) => {
    ...
    const btnIdentificar = page.locator('button:has-text("Identificar"):visible').first();
    ...
    const btnAgregarComp = page.locator('button:has-text("Agregar competencia"):visible');
    ...
    const trashButtons = page.locator('button[title*="Eliminar competencia"]:visible, button[title*="Limpiar texto"]:visible');
